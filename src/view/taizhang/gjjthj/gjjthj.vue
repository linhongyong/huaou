  ...
  <div>
    ...
    <Card>
			<div id="myElementId" style="width: 100%;" v-if="tableData.length">
				<div>
					<div style="text-align: center;font-size: larger;">钢筋接头（焊接）送检台账</div>
				...
				<table  class=""  border="1" rules="all">
					<tr class="">
						...
						<td class="" width="100">规格</td>
						<td class="" width="100">牌号</td>
						<td class="" width="100">炉批号</td>
						<td class="" width="140">送检日期</td>
						<td class="" width="200">送检数量（组）</td>
						<td class="" width="200">使用部位</td>
						<td class="" width="200">焊接方式</td>
						<td class="" width="150">检测报告编号</td>
						<td class="" width="100">检测结果</td>
						<td class="" width="100">其他</td>
					</tr>
					<tr v-for="(item, index) in tableData" :key="index">
						...
						<td class="">{{ item.specifications }}</td>
						<td class="">{{ item.steelGrade }}</td>
						<!-- <td class="">{{ item.amount }}</td> -->
						<td class="">{{ item.heatNo }}</td>
						...
						<td class="">{{ item.inspectionGroup }}</td>
						<td class="">{{ item.usePart }}</td>
						<td class="">{{ item.weldingMode }}</td>
...
// 				},2000)
      ...
      getDetail(id, okfn) {
        taizhangApi.getDetail({ id, type: "0002" })
          .then(data => {
           this.formItem = data
