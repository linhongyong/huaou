<template>
  <div>
    ...
    <Modal v-model="addModal.show" title="新增监理模板" ok-text="提交" footer-hide="true" width="60%">
    	<div id="" style="width:80%, margin:0 auto">
    		<Add  @addModalClose="addModalClose"></Add>

    	</div>
    </Modal>
    <Modal v-model="editModal.show" title="修改旁站监理模板" ok-text="提交" footer-hide="true" width="60%">
    	<div id="" style="width:80%, margin:0 auto">
...
import { getJxZkGzzPzjlTemplateList, deleteJxZkGzzPzjlTemplate, addJxZkGzzPzjlTemplate, getJxZkGzzPzjlTemplate } from '@/api/jxZkGzzPzjlTemplate'
export default {
  name: 'tables_page',
  components: {
    Tables, Add, Edit
...
//      {title: '站旁监理人员id', key: 'superId'},
  ...
  methods: {
    handleDelete (params) {
      console.log(params)
      let that = this;
      deleteJxZkGzzPzjlTemplate(params.row.id).then(res => {
      	console.log("--++++++++++++++++++++++++++++++++++++++++++++++");
      	console.log(res);
    ...
    editModalShow(params) {
    	...
    	console.log(params)
    	getJxZkGzzPzjlTemplate(params.row.id).then(res => {
	    	console.log("-------------------------");
